...
for i in range(side_len):
    for j in range(side_len):
        for di, dj in [(-1,-1), (-1,1), (1,-1), (1,1)]:
            ni, nj = i + di, j + dj
            if 0 <= ni < side_len and 0 <= nj < side_len:
                # Create a canonical pair representation to avoid duplicates
                pair = tuple(sorted([(i,j), (ni,nj)]))
                if pair not in processed_pairs:
                    processed_pairs.add(pair)
                    
                    # Get all possible crowns at both positions
                    pos1_crowns = [x[i,j,c] for c in range(side_len) if (i,j,c) in x]
                    pos2_crowns = [x[ni,nj,c] for c in range(side_len) if (ni,nj,c) in x]
                    
                    if pos1_crowns and pos2_crowns:
                        model.Add(sum(pos1_crowns) + sum(pos2_crowns) <= 1)
